{pkgs, ...}: let
  inherit (pkgs) lib stdenvNoCC fetchurl autoPatchelfHook versionCheckHook writableTmpDirAsHomeHook makeBinaryWrapper installShellFiles writeShellScript;
  ...
  manifestContents =
    if lib.pathExists manifestFile
    then lib.importJSON manifestFile
    else {};
  platforms = {

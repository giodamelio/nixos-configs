# jj — Jujutsu VCS config, the jj-push pre-push wrapper, and a starship prompt
# that swaps the git modules for a jj status module. Converted from
# nix/modules/home/jj.nix.
#   - `flake.lib.writeNushellApplication` -> `inputs.self.lib.*` via the
#     file-scope `inputs` closure (lib is a non-package flake value).
#   - `perSystem.jj-hunk.default` is a module arg from the per-system aspect.
{inputs, ...}: {
  den.aspects.jj.homeManager = {
    pkgs,
    perSystem,
    ...
  }: let
    jj-push = inputs.self.lib.writeNushellApplication pkgs {
      name = "jj-push";
      source = ''
        def main [...args] {

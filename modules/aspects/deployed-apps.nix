# deployed-apps — declarative deployment slots for self-contained app binaries
# (gio.deployedApps), plus the Gradient pull-deploy agent. Converted from
# nix/modules/nixos/deployed-apps.nix.
#
# Rewrites from the Blueprint original:
#   - `flake.nixosModules.reverse-proxy` in imports -> `includes` of the
#     converted reverse-proxy aspect.
#   - `flake.packages.${sys}.gradient-deployer` -> `perSystem.self.gradient-deployer`
#     (`perSystem` is a module arg from the per-system aspect).
{den, ...}: {
  den.aspects.deployed-apps = {
    includes = [den.aspects.reverse-proxy];
    nixos = {
      config,
      options,
      perSystem,
      ...
      gradientDeployer = perSystem.self.gradient-deployer;

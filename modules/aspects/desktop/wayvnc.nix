# wayvnc with socket activation and automatic idle shutdown. Converted from
# nix/modules/home/wayvnc.nix. flake.lib.writeNushellApplication ->
# inputs.self.lib (file-scope closure).
#
# Architecture:
...
{inputs, ...}: {
  den.aspects.wayvnc.homeManager = {pkgs, ...}: {
    ...
    systemd.user.services.wayvnc-idle-shutdown = {
      ...
      Service = {
        Type = "simple";
        ExecStart = let
          script = inputs.self.lib.writeNushellApplication pkgs {

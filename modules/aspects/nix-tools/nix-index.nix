# nix-index — command-not-found via the prebuilt nix-index database, plus a
# weekly DB refresh timer. Converted from nix/modules/home/nix-index.nix; the
# external nix-index-database input module is imported via the file-scope
# `inputs` closure.
{inputs, ...}: {
  den.aspects.nix-index.homeManager = {pkgs, ...}: let
